...
int main(int argc, char **argv)
{
        ...
        short              port_no;            // port to listen on
        ...
        // port_no = atoi(argv[1]);
        port_no = (short) strtol((const char *) (argv[1]), NULL, 10);
        port_no = port_no ? port_no : 8080;
        printf("Port: %d\n", port_no);
        ...
        while (1)
        {
                ...
                if ((stream = fdopen(child_fd, "r+")) == NULL)
                {
                ...
                else
                {
                        ...
                        { // child process
                                close(0); // close stdin
                                dup2(child_fd, 1); // map socket to stdout
                                dup2(child_fd, 2); // map socket to stderr
